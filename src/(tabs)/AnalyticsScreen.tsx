...
const AnalyticsScreen = () => {
    ...
    return (
      <View className="flex-row justify-around mb-4">
        {tabs.map(tab => (
          ...
          >
            <Text style={{ fontFamily: 'Raleway-Bold' }} className={selectedTab === tab ? 'text-white font-bold' : 'text-gray-700'}>{tab}</Text>
          </TouchableOpacity>
        ))}
      </View>
    );
  ...
  const renderInfoCard = (title: string, value: string | number) => (
    <View className="w-[48%] bg-white shadow-md p-4 rounded-xl mb-4">
      ...
      <Text style={{ fontFamily: 'Raleway-Bold' }} className="text-xl  mt-1">{value}</Text>
    </View>
  );
  ...
  const renderCragData = () => {
    ...
    return (
      <View className="mt-4">
        <Text style={{ fontFamily: 'Raleway-bold' }} className="text-lg  mb-2">Advanced Insights</Text>
        <View className="flex-row flex-wrap justify-between">
          {cragMetrics.map((item, index) => (
            ...
            >
              <Text style={{ fontFamily: 'Raleway-Semibold' }} className="text-sm text-gray-600 ">{item.label}</Text>
              <Text style={{ fontFamily: 'Raleway-Bold' }} className="text-xl text-gray-900  mt-1">{item.value}</Text>
            </View>
          ))}
        </View>
      </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <ScrollView className="flex-1 bg-gray-100 px-4 pt-10">
        <Text style={{ fontFamily: 'Raleway-Bold' }} className="text-2xl text-center mb-4">Business Analytics Dashboard</Text>
        ...
        {loading ? (
          <View className="items-center justify-center mt-10">
            ...
            <Text style={{ fontFamily: 'Raleway-Regular' }} className="text-sm mt-2 text-gray-500">Loading analytics...</Text>
          </View>
        ) : analyticsData ? (
          <>
            <View className="flex-row flex-wrap justify-between">
            ...
            <View className="bg-white p-4 mt-4 rounded-xl shadow mb-20">
              <Text style={{ fontFamily: 'Raleway-Bold' }} className="text-lg mb-2">Sales Over Time</Text>
          ...
          <Text style={{ fontFamily: 'Raleway-Regular' }} className="text-center text-gray-500 mt-10">No data found</Text>
        )}
