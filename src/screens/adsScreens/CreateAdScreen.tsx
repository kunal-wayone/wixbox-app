...
const validationSchema = Yup.object().shape({
  ...
  ad_type: Yup.string().required('Please select an ad type 🌟'),
  product_name: Yup.string().required('Please enter a product name 🛍️'),
        ...
        <View className="">
          ...
          <Formik
            initialValues={{
              ...
              ad_type: fetchedAdDetails?.ad_type || 'Promote Product',
            ...
            {({
              ...
              <View className="space-y-4">
                ...
                <View className="bg-white dark:bg-white rounded-lg p-4 mb-4" style={styles.shadow}>
                  ...
                  <TextInput
                    className="border border-gray-300 dark:border-gray-300 bg-gray-100 dark:bg-white rounded-lg p-3 text-base text-gray-800 dark:text-gray-200"
                ...
                <View className="bg-white dark:bg-white rounded-lg p-4 mb-4" style={styles.shadow}>
                  ...
                  {touched.product_name && errors.product_name && (
                    <Text className="text-red-500 text-xs mt-1">{errors.product_name}</Text>
                  )}
                </View>
                ...
                <View className="bg-white dark:bg-white rounded-lg p-4 mb-4" style={styles.shadow}>
                  ...
                  <View className="flex-col justify-between gap-2">
                    {[
                      { type: 'Promote Product', emoji: '🛍️', desc: 'Highlight a specific dish' },
                      { type: 'Special Offer', emoji: '🎉', desc: 'Discount or combo deal' },
                      { type: 'Event', emoji: '🎈', desc: 'Festival or special occasion' },
                    ].map(({ type, emoji, desc }) => (
                      <TouchableOpacity
                        key={type}
                        className={`flex-1 p-3 rounded-lg border ${values.ad_type === type ? 'bg-primary-100 border-gray-400' : 'bg-gray-100 dark:bg-white border-gray-300 dark:border-gray-300'}`}
                        onPress={() => setFieldValue('ad_type', type)}
                        accessibilityLabel={`Select ${type} ad type`}
                      >
                        <Text className={`text-left text-base font-semibold ${values?.ad_type === type ? "text-white dark:text-white" : "text-gray-900 dark:text-gray-900"}`}>{emoji} {type}</Text>
                        <Text className={`text-left text-xs  mt-1 ${values?.ad_type === type ? "text-white dark:text-white" : "text-gray-900 dark:text-gray-900"}`}>{desc}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                  {touched.ad_type && errors.ad_type && (
                    <Text className="text-red-500 text-xs mt-1">{errors.ad_type}</Text>
                  )}
                </View>
                ...
                <View className="bg-white dark:bg-white rounded-lg p-4 mb-4" style={styles.shadow}>
                  ...
                  <TextInput
                    className="border border-gray-300 dark:border-gray-300 bg-gray-100 dark:bg-white rounded-lg p-3 min-h-[100px] text-base text-gray-800 dark:text-gray-200"
                ...
                <View className="bg-white dark:bg-white rounded-lg p-4 mb-4" style={styles.shadow}>
                  ...
                  <View className="border border-gray-300 dark:border-gray-300 rounded-lg p-4 bg-gray-100 dark:bg-white">
                    ...
                    <View className="flex-row items-center mt-1">
                      <Text className="text-sm text-gray-600 dark:text-gray-400">{values.ad_type || 'Ad Type'}</Text>
                      <Text className="text-sm text-gray-800 dark:text-gray-800 ml-2">
                        {values.ad_type === 'Promote Product' ? '🛍️' : values.ad_type === 'Special Offer' ? '🎉' : '🎈'}
                      </Text>
                    </View>
                    {images.length > 0 && (
                      <Image source={{ uri: images[0].uri }} className="w-full h-48 rounded-lg mt-2" resizeMode="cover" accessibilityLabel="Ad preview image" />
                    )}
                    <Text className="text-base text-gray-800 dark:text-gray-200 mt-2">
