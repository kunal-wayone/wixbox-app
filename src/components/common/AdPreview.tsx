    ...
    return (
        <View className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-6" style={styles.shadow}>
            <View className="flex-row items-center mb-3">
                ...
                <Text className="text-base font-semibold ml-2 text-gray-900 dark:text-white">Ad Preview</Text>
            </View>

            <View className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                <Text numberOfLines={1} ellipsizeMode='tail' className="text-xl font-bold text-gray-900 dark:text-white mb-1">{ad_title}</Text>

                <View className="flex-row items-center mb-2">
                    <Text numberOfLines={1} ellipsizeMode='tail' className="text-sm text-gray-600 dark:text-gray-300">{ad_type}</Text>
                    <Text className="ml-2 text-lg">{getAdTypeIcon()}</Text>
                </View>
                ...
                {previewImage ? (
                    ...
                    <View className="w-full h-48 rounded-lg bg-gray-200 dark:bg-gray-600 justify-center items-center mb-3">
                        ...
                        <Text className="text-xs text-gray-500 mt-1">No image uploaded</Text>
                    </View>
                )}

                <Text numberOfLines={2} ellipsizeMode='tail' className="text-base text-gray-800 dark:text-gray-900 mb-3">{caption}</Text>
                ...
                <View className="flex-row justify-between mt-2">
                    <Text className="text-sm font-semibold text-gray-800 dark:text-white">
                        ...
                        {discounted_price ? (
                            <Text className="text-sm text-red-500"> → ${discounted_price}</Text>
                        ) : null}
                    </Text>
                    <Text className="text-sm font-semibold text-gray-800 dark:text-white">
                        Budget: ${budget} 💸
